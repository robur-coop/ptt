  ...
  let encode_without_tls ctx v w =
    let rec go = function
      | Colombe.State.Read { k; buffer; off; len } ->
          Colombe.State.Read { k= Fun.compose go k; buffer; off; len }
      | Colombe.State.Write { k; buffer; off; len } ->
          Colombe.State.Write { k= Fun.compose go k; buffer; off; len }
      | Colombe.State.Return v -> Return v
      | Colombe.State.Error err -> Error err
    in
...
module Value_with_tls = struct
  include Sendmail_with_starttls.Make_with_tls (Value)
...
let m_relay_init ctx info =
  ...
  | None ->
      let open Monad in
      send ctx Value.PP_220 [ Colombe.Domain.to_string info.domain ]
  ...
  | Some tls ->
      ...
      let capabilities =
        ...
        ; "SMTPUTF8"; "STARTTLS"; Fmt.str "SIZE %d" info.size
        ]
      in
      ...
      let rec go () =
        if !reset >= 25 && !bad >= 25 then
          ...
          | `Verb ("STARTTLS", []) ->
              ...
              let decoder =
                Sendmail_with_starttls.Context_with_tls.decoder ctx
              in
          ...
          | _ ->
              incr bad;
              let msg = [ "Must issue a STARTTLS command first." ] in
              let* () = send ctx Value.PN_530 msg in
              go ()
      in
      go ()
