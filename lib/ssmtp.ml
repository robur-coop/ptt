    ...
    let fiber : a send -> [> Encoder.error ] Encoder.state = function
      | Payload ->
          let k encoder =
            ...
            Encoder.write "\r\n" encoder;
            Encoder.flush (fun _ -> Encoder.Done) encoder
          in
      ...
      | Code ->
          let code, txts = v in
          Reply.Encoder.response (`Other (code, txts)) encoder
    in
    let rec go = function
      | Encoder.Done -> Return ()
      | Encoder.Write { continue; buffer; off; len } ->
          Write { k= go <.> continue; buffer; off; len }
      | Encoder.Error err -> Error err
    in
    (go <.> fiber) w
...
let m_submission_init ctx info ms =
  let open Monad in
  let* () = send ctx Value.PP_220 [ Colombe.Domain.to_string info.domain ] in
